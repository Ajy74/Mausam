...
class CityCurrent{
  ...
  String state;
  String country;
  CityCurrent({required this.city,required this.state,required this.country});

  late var data;

  Future<ForCastModel> fetchForcast() async{
   
    final response = await http.get(Uri.parse("${ApiEndPoints.forcastWeatherUrl}&q=${city}-${state}-${country}&days=7&aqi=no&alerts=yes"));
